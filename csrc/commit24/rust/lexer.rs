...
pub enum V{
    Int(i32),
    Str(String), //step9：2023/05/05 感觉这里用&str不优雅，后续看看能不能用enum嵌套或者String
    // step21: 丑陋的就是不正确的，&str类型会带来很多生命周期问题，果断换掉
}
#[allow(dead_code)]
#[derive(Debug, PartialEq, Clone)]
// 终结符结构体
...
fn is_keywords(token: &Token) -> bool{
    let keys = vec!["return", "if", "else", "for", "while", "int"];
    if let Some(V::Str(st)) = &token.value{
        if keys.contains(&st.as_str()){
            return true;
        }else{
            return false
        }
        ...
        match c {
            ...
            c if c.is_ascii_punctuation() => {
                ...
                let token = Token {
                    ...
                    value: Some(V::Str(str1.to_owned())),
                    loc: i,
                    len: punct_len,
                };
                tokens.push(token);
            ...
            'a'..='z' | 'A'..='Z' | '_'  => {
                ...
                let token = Token {
                    ...
                    value: Some(V::Str(ident.to_owned())),
                    loc: i,
                    len: end - start,
                };
                tokens.push(token);
