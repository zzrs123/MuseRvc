...
use crate::ast::{Node,NodeKind,Function, Program};
use crate::error;
...
/*====================================================================
 ...
 pub fn parser(tokens:  Vec<Token>) -> Result<Program, String> {
    ...
    while tokens[pos].kind != TokenKind::TKEof{
        let func1 =  function(&tokens, &mut pos);
        match func1 {
            Ok(func1) => {
                // 处理 Function 类型的返回值
                prog.funclist.push(func1);
            },
            Err(err_msg) => {
                // 处理 String 类型的错误信息
                error!(pos, "{}",err_msg);
...
pub fn function(tokens: &Vec<Token>, mut pos: &mut usize) -> Result<Function, String>{
    ...
    ty = declarator(tokens, pos, base_ty)?;

    let mut func = Function::new();
    func.name = get_ident(&ty.name.unwrap());
    // 这里实际上是跳过"{"
    // *pos += 1; // 所以改写为下面的方式
...
fn type_suffix(tokens: &Vec<Token>, pos: &mut usize, ty: Type) -> Result<Type, String> {
    if equal(&tokens[*pos], "("){
        *pos += 1;
        // let rest = skip(tokens, pos, ")");
        if equal(&tokens[*pos], ")"){
            *pos+=1;
            let func_ty = Type::func_type(ty);
            return Ok(func_ty); 
        }else{
            let loc = *pos;
            error!(loc,"expect {}", ")");
    ...
    match tmp {
        ...
        tmp if tmp.unwrap().kind == TokenKind::TkIdent => {
            ...
            if equal(&tok, "("){
                ...
                // *pos+=2; //这里是跳过 ')'
                // return Ok(node);
                return funccall(tokens, pos, func);
            }
            *pos+=1;
            let var_name;
...
/*====================================================================
 ...
 ====================================================================*/
fn funccall(tokens: &Vec<Token>, pos: &mut usize, func:&mut Function) -> Result<Box<Node>, String> {
    let mut head = Node::new_node(NodeKind::NdEmpty);
    ...
    *pos+=2;
    // let tok = &tokens[*pos];
    while !equal(&tokens[*pos], ")") | equal(&tokens[*pos], ";"){
        if cur != &tmp_head {
            *pos+=1;
        }
        // assign
        cur.next = Some(assign(tokens, pos, func)?);
        cur = cur.next.as_mut().unwrap();
    }
    ...
    let mut node = Node::new_node(NodeKind::NdFuncall);
    // ident
    if let Some(V::Str(st))= &start.clone().value{
        node.funcname = st.to_string();
    }
